...
(function() {
	...
	Evented = function() {
		...
		Evented.prototype.off = function(event, handler) {
			...
			if (handler == null) return delete this.bindings[event];
				...
				i = 0;
				_results = [];
				while (i < this.bindings[event].length) if (this.bindings[event][i].handler === handler) _results.push(this.bindings[event].splice(i, 1));
	...
	shouldTrack = function(method) {
		...
		if (!ignoreStack.length && options.ajax) if (method === "socket" && options.ajax.trackWebSockets) return true;
		...
		function d(D, O) {
			...
			function C() {
				if (location.hash && location.hash.length > 1) {
					...
					if (aL.length && Y.find(aK)) if (am.scrollTop() === 0) aJ = setInterval(function() {
						if (am.scrollTop() > 0) {
							ab(aK, true);
...
(function(A) {
	...
	A.fn.marquee = function(E) {
		...
		if (C && F && this.length) if (F.toLowerCase() == "object") return C;
		else if (C[F]) {
			var B;
			this.each(function(G) {
				var H = A.data(this, "marquee")[F].apply(C, D);
				if (G == 0 && H) if ( !! H.jquery) B = A([]).add(H);
				else {
					B = H;
					return false
	...
	function initHeader() {
		...
		for (var i = 0; i < points.length; i++) {
			...
			for (var j = 0; j < points.length; j++) {
				...
				if (!(p1 == p2)) {
					var placed = false;
					for (var k = 0; k < 5; k++) if (!placed) if (closest[k] == undefined) {
						closest[k] = p2;
						placed = true
					}
					for (var k = 0; k < 5; k++) if (!placed) if (getDistance(p1, p2) < getDistance(p1, closest[k])) {
						closest[k] = p2;
						placed = true
					}
				}
